...
| Environment layer | Minimal but runnable | 40% | Local plus existing Conda/micromamba wrappers are implemented with explicit runner/env selection, env-file hash provenance, and `env check` readiness probes. No env creation, package solving, Docker, Singularity, or SLURM. |
...
5. Deepen environment management:
   - add explicit `agentflow env prepare` for Conda/micromamba after the check path is stable
