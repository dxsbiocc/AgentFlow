...
## 11. 护栏决议（已与维护者敲定 2026-06-12）

1. **阈值 / 已验证使用**：双触发——(a) ≥3 次**已验证使用**(=跑了+产出真实发现+grounded 通过)，或 (b) B 检出输出领域 mismatch + 存在 ≥1 近邻工具即够格(实锤挡住的真任务，非投机复发)。失败/错配的跑不计入。
2. **相似判定**：分层而非平铺 AND——① 硬门:I/O 签名兼容(确定性，先剪枝，不兼容直接否)；② LLM 在 I/O 兼容集内确认"同领域动作"；③ 使用证据(复发数/mismatch)。LLM 只在结构约束集内确认。
3. **治理门(已定:分级治理)**：exploratory→exploratory 且验证门过、不淘汰在跑工具 → **自动提升(仅记录、不阻塞)**；会 deprecate 在跑工具 或 晋升 verified → **人工确认(阻塞 handoff)**。对齐 A3"便宜可逆自动、重要才拦"。
4. **变异点**：允许多槽，但每槽独立证据门(确是硬编码/未暴露默认 且 在证据中变化过或被 mismatch 所需)。不投机参数化一切。
5. **验证门覆盖(已定:原始+新+抽样K)**：必验 原始特化案例(回归证无损) + 新触发案例(证新能力) + 最多 K(默认2-3)个代表性 subsumed 案例；任一失败即拒绝泛化。verified 晋升可选全覆盖。
6. **失败回退**：泛化验证失败 → 不泛化、保留特化原样、对被挡任务降级为合成全新特化；记录失败原因(变异点比一个参数更深)避免静默重试；只在成功时扬弃。
7. **OutputGroundingScorer 接法**：独立 trait(Noop 默认+可测) 但复用项目已配置的同一 LLM 后端(走 RelevanceScorer 同 wiring)，不引入新 LLM 配置。

## 12. 落地决定

- **首阶段 = AS15（输出领域校验先行）**：OutputGroundingScorer(Noop 默认) + 在 `raise_stance_assessment_for_observation` 前读真实报告内容做 grounding 校验 + mismatch 拒绝为证据 + 诚实 apply_failure/handoff + 埋好自纠正触发点。**不含进化**，立即止"蒙头用"之血。
- 后续 AS16(复发检测，只看不改) → AS17(泛化+验证门) → AS18(扬弃谱系+自纠正分叉接线)，按 §9，每阶段保持 §8 不变量。

---

**RFC 护栏已敲定（§11）。** 落地严格按 §9/§12 分阶段，每阶段保持 §8 不变量。
